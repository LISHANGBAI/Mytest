<template>
	<div class="main-content" :style='{"padding":"0px 30px 30px","fontSize":"14px","color":"#666","background":"none"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('zhiyuanzhefuwu','接取需求')" @click="jiequxuqiuCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
							...
							<el-button class="view" v-if="isAuth('zhiyuanzhefuwu','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
		...
		<jiequxuqiu-cross-add-or-update v-if="jiequxuqiuCrossAddOrUpdateFlag" :parent="this" ref="jiequxuqiuCrossaddOrUpdate"></jiequxuqiu-cross-add-or-update>
...
import jiequxuqiuCrossAddOrUpdate from "../jiequxuqiu/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				jiequxuqiuCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			jiequxuqiuCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.jiequxuqiuCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','zhiyuanzhefuwu');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.jiequxuqiuCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.jiequxuqiuCrossaddOrUpdate.init(row.id,type);
				});
			},
	...
	this.$router.push({path:'/discusszhiyuanzhefuwu',query:{refid:id}});
    },
    // 下载
    download(file){
