	...
	<div class="addEdit-block" :style='{"padding":"0px 0px 30px","fontSize":"14px","color":"#000","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="upload" v-if="type!='info' && !ro.zhengjianzhao" label="证件照" prop="zhengjianzhao">
					<file-upload
						tip="点击上传证件照"
						action="file/upload"
						:limit="3"
						:multiple="true"
						:fileUrls="ruleForm.zhengjianzhao?ruleForm.zhengjianzhao:''"
						@change="zhengjianzhaoUploadChange"
					></file-upload>
				</el-form-item>
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="upload" v-else-if="ruleForm.zhengjianzhao" label="证件照" prop="zhengjianzhao">
					<img v-if="ruleForm.zhengjianzhao.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.zhengjianzhao.split(',')[0]" width="100" height="100">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.zhengjianzhao.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="select" v-if="type!='info'"  label="服务等级" prop="fuwudengji">
					<el-select :disabled="ro.fuwudengji" v-model="ruleForm.fuwudengji" placeholder="请选择服务等级" >
						<el-option
							v-for="(item,index) in fuwudengjiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="服务等级" prop="fuwudengji">
					<el-input v-model="ruleForm.fuwudengji"
						placeholder="服务等级" readonly></el-input>
				</el-form-item>
			</template>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='zhengjianzhao'){
							this.ruleForm.zhengjianzhao = obj[o];
							this.ro.zhengjianzhao = true;
							continue;
						}
						if(o=='dianhuahaoma'){
				...
				this.ruleForm.shijianbi = Number('0'); 
            ...
            this.fuwudengjiOptions = "高,中,低".split(',')
	...
	if(this.ruleForm.zhengjianzhao!=null) {
		this.ruleForm.zhengjianzhao = this.ruleForm.zhengjianzhao.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
    ...
    zhengjianzhaoUploadChange(fileUrls) {
	    this.ruleForm.zhengjianzhao = fileUrls;
    },
  }
};
</script>
